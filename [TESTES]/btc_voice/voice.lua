...
addEventHandler("onClientPreRender", root,
    function ()
       ...
       for k, v in ipairs(players) do  
        ...
        if getElementData(localPlayer, "inCall") == true then 
            setSoundVolume(localPlayer, 1)
            setSoundVolume(v, 1)
            return 
        end
            ...
            local fMaxVol = v:getData("maxVol") or 4
            local fMinDistance = v:getData("minDist") or 5
            local fMaxDistance = 10
            ...
            if isLineOfSightClear(localPlayer.position, vecSoundPos, true, true, false, true, false, true, true, localPlayer) then -- line of sight clear
               ...
               -- setSoundEffectEnabled(v, "parameq", false)
              --  setSoundEffectEnabled(v, "chorus", false)
                ...
                setSoundEffectEnabled(v, "chorus", true)
                setSoundEffectEnabled(v, "parameq", true)
...
addEventHandler("onClientElementStreamIn", root,
    function ()
        if source:getType() == "player" then
           if getElementData(source, "inCall") == false then
