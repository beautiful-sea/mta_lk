...
local partTable = {{"windscreen_dummy", "pára brisas"},
{"bump_front_dummy", "Amortecedor dianteiro"},
{"bump_rear_dummy", "Amortecedor traseiro"},
{"bonnet_dummy", "Capô"},
{"boot_dummy", "Malas"},
{"door_lf_dummy", "Porta da frente esquerda"},
{"door_rf_dummy", "Porta da frente direita"},
{"door_lr_dummy", "Porta da frente direita"},
{"door_rr_dummy", "Porta traseira direita"},
{"wheel_rf_dummy", "Roda dianteira direita"},
{"wheel_lf_dummy", "Roda dianteira esquerda"},
{"wheel_rb_dummy", "Roda traseira direita"},
{"wheel_lb_dummy", "Roda traseira esquerda"},
{"wheel_rear", "Roda traseira"},
{"wheel_front", "Roda dianteira"}}
...
local function getPartImage(part)
	...
	if part then
		if string.find(part, "pára brisas") then
			returnVal = "icons/windscreen.png"
		elseif string.find(part, "Porta") then
			returnVal = "icons/door.png"
		elseif string.find(part, "Malas") then
			returnVal = "icons/trunk.png"
		elseif string.find(part, "Capô") then
			returnVal = "icons/hood.png"
		elseif string.find(part, "Amortecedor dianteiro") then
			returnVal = "icons/frontb.png"
		elseif string.find(part, "Amortecedor traseiro") then
			returnVal = "icons/backb.png"
		elseif string.find(part, "Roda") then
			...
			for k in pairs(components) do
				if getNameFromPart(k) then
					...
					if x then
						...
						if getNameFromPart(k) == "Capô" and scx and not engineBackVeh[getElementModel(v)] then
...
addEventHandler("onClientClick", root, function(button, state)
	...
	if getElementData(vehicleRepair, "veh:owner") == getElementData(localPlayer, "char:id") then 
		outputChatBox("#FFA000[BTC ERROR] #FFFFFFVocê não pode reparar seu próprio veiculo.", 255,255,255, true)
		return 
		end

		
		if (getElementData(localPlayer, "maleta:area") == false) then
		outputChatBox("#FFA000[BTC ERROR] #FFFFFFSua maleta está muito longe para reparar o veiculo.", 255,255,255, true)
		else
	if overPart then
		local cost = defPrice
		...
		if tonumber(getElementData(localPlayer, "char:money")) < cost then
			outputChatBox("Você não tem dinheiro suficiente para consertar. necessário: $"..tostring(cost))
			return
		end
