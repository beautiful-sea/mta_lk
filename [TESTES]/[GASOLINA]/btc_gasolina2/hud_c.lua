...
--			dxDrawShadowText (  mph.."MPH | "..kph.."KPH", recX, recY,  recX + 180, recY + 40, tocolor ( 255, 255, 255, 255 ), fontSize, "default-bold", "center", "center" )
			dxDrawShadowText (kph.."KM/H", recX, recY,  recX + 180, recY + 40, tocolor ( 255, 255, 255, 255 ), fontSize, "default-bold", "center", "center" )
		...
		if ( vd[2] ) then
			...
			if ( isElement ( car ) ) then
				...
				if ( fuel2 <= 0 and getVehicleEngineState ( car ) ) then
					...
					exports.GTIhud:dm("Seu veiculo esta sem gasolina!", 255, 125, 0)
			...
			--dxDrawShadowText (  tostring ( someData.vehicleFuel ).."% Gasolina", recX, sy - 70, recX + 85, ( sy - 70 ) + 40, tocolor ( 255, 255, 255, 255 ), fontSize, "default-bold", "center", "center" )
			dxDrawShadowText ("Gasolina", recX, sy - 70, recX + 85, ( sy - 70 ) + 40, tocolor ( 255, 255, 255, 255 ), fontSize, "default-bold", "center", "center" )
		...
		if ( vd[3] ) then
			...
			--dxDrawShadowText (  tostring ( someData.vehicleHealth ).."% Vida", recX+95, sy - 70, recX + (100+85), ( sy - 70 ) + 40, tocolor ( 255, 255, 255, 255 ), fontSize, "default-bold", "center", "center" )
			dxDrawShadowText ("Vida", recX+95, sy - 70, recX + (100+85), ( sy - 70 ) + 40, tocolor ( 255, 255, 255, 255 ), fontSize, "default-bold", "center", "center" )
...
function dxDrawSpeedo ()
		local car = getPedOccupiedVehicle ( localPlayer )
			if ( isElement ( car ) ) then
				fuel = getElementData ( car, "veh:fuel" )				

				if not fuel then
					setElementData ( car, "veh:fuel", 75 )
				end

				local frontLeft, rearLeft, frontRight, rearRight = getVehicleWheelStates ( car )

				if frontLeft == 1 and getVehicleEngineState ( car ) then 
					exports.btc_hud:dm("SEU VEICULO ESTÁ COM O PNEU FURADO! CHAME O MECANICO!", 255, 125, 0)
					setVehicleEngineState ( car, false )
					return
				end
				if rearLeft == 1 and getVehicleEngineState ( car ) then 
					exports.btc_hud:dm("SEU VEICULO ESTÁ COM O PNEU FURADO! CHAME O MECANICO!", 255, 125, 0)
					setVehicleEngineState ( car, false )
					return
				end
				if frontRight == 1 and getVehicleEngineState ( car ) then 
					exports.btc_hud:dm("SEU VEICULO ESTÁ COM O PNEU FURADO! CHAME O MECANICO!", 255, 125, 0)
					setVehicleEngineState ( car, false )
					return
				end
				if rearRight == 1 and getVehicleEngineState ( car ) then 
					exports.btc_hud:dm("SEU VEICULO ESTÁ COM O PNEU FURADO! CHAME O MECANICO!", 255, 125, 0)
					setVehicleEngineState ( car, false )
					return
				end
				...
				local fuel2 = math.floor ( fuel )
				if ( tonumber(fuel2) <= 0 and getVehicleEngineState ( car ) ) then
					setVehicleEngineState ( car, false )
					exports.btc_hud:dm("Seu veiculo esta sem gasolina!", 255, 125, 0)
				end
		end
end
addEventHandler ( "onClientPreRender", root, dxDrawSpeedo )