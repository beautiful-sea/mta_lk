	...
	function ( hitElement, matchingDimension )
		if getElementType(hitElement) == "player" and (hitElement == localPlayer) then
			...
			if not theVehicle then
				...
				if getElementData(theVehicle, "owner") == getElementData(localPlayer, "dbid") then
		...
		for index, value in ipairs (getElementsByType("vehicle")) do
			if getElementData(value, "dbid") == tonumber(vehicleId) then 
				if not getElementData(value, "owner") == getElementData(localPlayer, "dbid") then 
					outputChatBox("#7cc576[LK MTA] #ffffffVocê não é o dono do veículo!",0,0,0,true)	
						return
					end
					if getElementData(localPlayer, "spawn:vehicle") then
						outputChatBox("#7cc576[LK MTA] #ffffffVocê precisa guardar seu antigo veiculo para spawnar outro.",0,0,0,true)
						return
					end
					...
					if (getElementData(value, "prfAP")) then 
						outputChatBox("#7cc576[LK MTA] #ffffffSeu veiculo está apreendido na PRF!",0,0,0,true)
					...
					if (getElementData(value, "detranAP")) then 
						...
						if not vehicleWeight[getElementModel(value)] then
							outputChatBox("#7cc576[LK MTA] #ffffffSeu veiculo está no detran!",0,0,0,true)
							triggerServerEvent("updateINTDIM22", localPlayer, vehicleId)
						else
						...
						if not vehicleWeight[getElementModel(value)] then
							...
							outputChatBox("#7cc576[LK MTA] #ffffffnão pode spawnar caminhão nesta garagem!!!",0,0,0,true)
						end
					end
				end
			end
			...
			if currentPage == 1 then
				...
				if #myVehicles > 0 then
					...
					for key, value in ipairs(myVehicles) do
						...
						triggerServerEvent("respawnVehicle",root,value)
						if key >= currentVehicleRow and key <= lastVehicleRow then
							key = key - currentVehicleRow + 1
							local forY = startY + 2 * spacerBig + 30 + spacer + (key - 1) * 22
							
							dxDrawText(exports["vehicle-system"]:newNames(getElementModel(value)).." (ID: "..getElementData(value, "dbid")..")", startX + 3 * spacerBig + spacer, forY, startX + 3 * spacerBig + spacer + width / 2 - 4 * spacerBig - 2 * spacer, forY + 20, tocolor(255, 255, 255), 1, roboto, "left", "center")
							...
							if isCursorHover(startX + 2 * spacerBig + spacer, forY, width / 2 - 4 * spacerBig - 2 * spacer, 20) or key == selectedVehicle - currentVehicleRow + 1 then
								dxDrawText(exports["vehicle-system"]:newNames(getElementModel(value)).." (ID: "..getElementData(value, "dbid")..")", startX + 3 * spacerBig + spacer, forY, startX + 3 * spacerBig + spacer + width / 2 - 4 * spacerBig - 2 * spacer, forY + 20, tocolor(0, 0, 0), 1, roboto, "left", "center")
