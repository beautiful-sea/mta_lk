local font1 = dxCreateFont("font/fonte.ttf", 10)
local font_10 = dxCreateFont("font/fonte.ttf", 10)
        ...
        if #dxGrid.items > 0 then
            ...
            for index, data in ipairs ( dxGrid.items ) do
                ...
                if data.vis then
                    ...
                    dxDrawRectangle (data.x, data.y, data.w, data.h, tocolor ( 0, 0, 0, 55), data.pg );

                    -- Draw the column bar
                    dxDrawRectangle ( data.x, data.y, data.w, 30 % data.h, tocolor (0, 112, 175, 255), data.pg );
                    ...
                    if data.se and data.se <= data.r and data.se >= seeFrom and data.se <= seeTo then
                        ...
                        dxDrawRectangle ( data.x, ( 30 % data.h ) + y2, data.w, 25, tocolor (0,0,0, 50 ), data.pg );
                    ...
                    local cWidth = 0
                        ...
                        for cIndex, cData in ipairs ( data.i ) do
                            ...
                            if ( ( cWidth + cData.info.width ) <= data.w ) then
                                ...
                                dxDrawText ( cData.info.title, x + 10, data.y, cData.info.width + x, ( 30 % data.h ) + data.y, tocolor ( 255, 255, 255 , 220 ), FIT_MODE and ( 1 * SCALE ) or 1, font_10, "left", "center", true, true, data.pg, false, true );
                                ...
                                if #cData > 0 then
                                    ...
                                    for iIndex = seeFrom, seeTo do
                                        ...
                                        if cData[iIndex] then
                                            ...
                                            -- Draw the item text
                                            dxDrawText ( cData[iIndex]["text"], x + 20, y, cData.info.width + x, ( 30 % data.h ) + y + 5, tocolor ( 0, 0, 0, 70), FIT_MODE and ( 1 * SCALE ) or 1, font_10, "left", "center", true, true, data.pg, false, true );
                                        end
                                    end
                                end
