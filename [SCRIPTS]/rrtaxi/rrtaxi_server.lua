...
function enterVehicle ( thePlayer, seat, jacked )
    if ( taxis[getElementModel ( source )] ) and ( not chauffeurSkins[getElementModel ( thePlayer )] ) and (seat == 0) then 
		cancelEvent()
        outputChatBox ( "Only taxi drivers can drive taxis!", thePlayer )
    end
end
						...
						if actualspeed < 0.25 then
							...
							for seat = 0, seats do
								...
								if not occupant and tonumber(freeSeats) == 0 then
									...
									outputChatBox ( "Drive your passenger to the destination marker.", playerSource );
								end
							end
...
function quitJob ( playerSource )
	...
	outputChatBox ( "Job canceled.", playerSource );
	else
	outputChatBox ( "You don't have an assignment!", playerSource );
	end
end
...
addCommandHandler ("addtaxi",pickup)