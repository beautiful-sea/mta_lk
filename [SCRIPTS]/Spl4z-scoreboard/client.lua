...
local SCOREBOARD_HEADER_HEIGHT		= 45				-- Height for the header in what you can see the server info
...
local SCOREBOARD_INFO_BACKGROUND	= { 0,0,0, 230 }			-- RGBA color for the info header background
...
local SCOREBOARD_BACKGROUND		= { 0,0,0, 150}			-- RGBA color for the background
...
drawScoreboard = function ()
	...
	local messageAdmin = getElementData(getLocalPlayer(),"motd") or "Message is not set."
	messageAdmin = tostring ( messageAdmin )	
	...
	dxDrawText ( messageAdmin, left, top, right, bottom,
				 SCOREBOARD_SERVER_NAME_COLOR, 1, "default-bold", "left", "bottom",
				 true, false, SCOREBOARD_POSTGUI )				 
	...
	local renderEntry = function ( player )
		...
		local playerPing = math.ceil(getPlayerPing ( player )/1.3)
