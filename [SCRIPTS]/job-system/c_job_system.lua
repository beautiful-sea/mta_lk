...
--[[local jessie = createPed( 141,297.326171875, 82.81, 1029.3762207031)
setPedRotation( jessie, 3.38 )
setElementDimension( jessie, 2 )
...
function showEmploymentWindow()
	...
	--PHOTO
	local row = guiGridListAddRow(jobList)
	guiGridListSetItemText(jobList, row, column, "Photographer", false, false)
	...
	-- PIZZA
	--local row = guiGridListAddRow(jobList)
	--guiGridListSetItemText(jobList, row, column, "Pizza Delivery", false, false)
	...
	if (button=="left") then
		...
		elseif (job>0) then
			...
			elseif (jobtext=="Locksmith") then
				displayLocksmithJob()
				job = 6
			elseif (jobtext=="Photographer") then
				job = 8
			elseif (jobtext=="Pizza Delivery") then
				 displayPizzaJob()
				job = 9
			end
