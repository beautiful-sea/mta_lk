		...
		if(bus[vehID])then
			if(seat~=0)then
				...
				if driver then -- you can only pay the driver if the bus has a driver
					if not exports.global:hasMoney(getLocalPlayer(), 10)then
						triggerServerEvent("removePlayerFromBus", getLocalPlayer())
						outputChatBox("You can't afford the $10 bus fare.", 255, 0, 0)
					else
						triggerServerEvent("payBusFare", getLocalPlayer(), driver)
						outputChatBox("You have paid $10 to ride the bus", 0, 255, 0)
					end
				end
