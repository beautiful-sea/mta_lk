...
function givePlayerJob(jobID)
	...
	mysql:query_free("UPDATE characters SET job=" .. mysql:escape_string(jobID) .. ", jobcontract = 1 WHERE id = " .. mysql:escape_string(getElementData(source, "dbid")) )
	...
	if (jobID==8) then -- PHOTOGRAPHER
		exports.global:giveWeapon(source, 43, 150, true)
		outputChatBox("Use this camera to take photos.You can sell them later at SAN headquarter.'S' on city map", source, 255, 194, 14)
	end
end
