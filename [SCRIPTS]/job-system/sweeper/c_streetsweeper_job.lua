...
function displaySweeperJob()
	if (jobstate==0) then
		jobstate = 1
		blip = createBlip(1425.7353515625, -1837.5615234375, 13.546875 , 0, 4, 255, 127, 255)
	...
	if not (vehicle) then
		...
		if (model==414) then -- MULE
			if (jobstate==3) then
				...
				outputChatBox("You earned $" .. wage .. " on your sweeping runs.", 255, 194, 15)
				local vehicle = getPedOccupiedVehicle(localPlayer)
			...
			outputChatBox("Thank you for your services.", 0, 255, 0)
			destroyElement(colshape)
			destroyElement(marker)
			destroyElement(blip)
