...
function mainWindow ()
	if not getElementData(getLocalPlayer(), "adminjailed") then
		...
		if guiGetVisible(gameWindow)== true then 
			guiSetVisible(gameWindow,false)
			triggerServerEvent("updatescore", getLocalPlayer(), currentScore)
			showCursor(false)
		end
		outputChatBox("This feature is only for players in admin jail!",255,0,0,true)
		return
	end
...
function moveit ()
	...
	if headX < -0.01 or headX >= 0.99 or headY < -0.01 or headY >= 0.99 then
		...
		triggerServerEvent("updatescore", getLocalPlayer(), currentScore,2)
