...
function knockHouse()
	triggerServerEvent("onKnocking", getLocalPlayer(), houseID)
	hideHouseMenu()
end
...
--[[		for key, value in ipairs(getElementsByType("pickup")) do
		...
		for key, value in ipairs(getElementsByType("pickup")) do
			if isElementStreamedIn(value) then
				...
				if (wx >= minx and wx <=maxx) and (wy >= miny and wy <=maxy) and (wz >= minz and wz <=maxz) then
					local dbid = getElementData(value, "dbid")
					if getElementParent( getElementParent( value ) ) == interiorres then -- house found
						element = value
						id = dbid
						break
					elseif getElementData( value, "other" ) and getElementParent( getElementParent( value ) ) == elevatorres then
						-- it's an elevator
						if getElementDimension( value ) then
							element = value
							id = getElementDimension( value )
							break
						elseif getElementDimension( getElementData( value, "other" ) ) then
							element = value
							id = getElementDimension( getElementData( value, "other" ) )
							break
						end
	...
	function( other, rot )
		...
		inttimer = setTimer(function(localPlayer, thePickup, other)
			...
			if isElement(localPlayer) then
				...
				-- fade camera in
				setTimer(fadeCamera, 50, 1, true, 2)
				setTimer(setElementFrozen, 100, 1, localPlayer, false )
			end
		end, 50, 1, localPlayer, thePickup, other)
	end
)
