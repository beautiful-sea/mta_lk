	...
	if (attacker) and (weapon == 3) then	
		if getElementParent(attacker) == kingCOP then
			...
			if (getElementData ( theprisoner, "currentstatus" ) ~= "underarrest") and (wlevel > 0) then	
				...
				outputChatBox("You are getting arrested by "..getPlayerName(attacker).." Do not quit/logoff to avoid arrest",player,255,0,0)
			end
		end
	end
end
...
function donotpassgo(player, mx, my, mz)
	...
	time = jailtime/1000
	outputChatBox("You got arrested for "..jailtime.." seconds by a cop",player,0,0,255)
