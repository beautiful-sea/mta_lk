...
function stevieIntro (thePlayer) -- When player enters the colSphere create GUI with intro output to all local players as local chat.
	...
	if(getElementData(stevie, "activeConvo")==1)then
		...
		exports.logs:logMessage("[STEVIE] " .. getElementData(source, "gameaccountusername") .. "/".. getPlayerName(source) .." met Stevie" , 25)
...
function stevieSuccess_S()
	...
	exports.logs:logMessage("[STEVIE] " .. getElementData(thePlayer, "gameaccountusername") .. "/".. getPlayerName(thePlayer) .." got Stevie' his businesscard" , 25)
end
					...
					if (phoneState==1) then -- is someone already speaking to stevie?
						...
						if not(steviesFriend==1) or (factionType==4) or (factionType==2) or (factionType==3) then
							...
							elseif stevieMarker then
								...
								exports.logs:logMessage("[STEVIE] " .. getElementData(thePlayer, "gameaccountusername") .. "/".. getPlayerName(thePlayer) .." called Stevie" , 25)
							end
						end
					end
				end
...
function givePlayerStevieItems(thePlayer, veh, deal)
	if not(isElementWithinColShape(veh, collectionCol)) then
		...
		if not(exports.global:hasSpaceForItem(veh))then
			...
			exports.logs:logMessage("[STEVIE] " .. getElementData(thePlayer, "gameaccountusername") .. "/".. getPlayerName(thePlayer) .." [Weapon" .. strisweapon .."] [itemID: ".. itemID .."] [value: ".. value .."]" , 25)
		end
	end
end
