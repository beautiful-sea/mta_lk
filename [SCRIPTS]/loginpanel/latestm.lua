...
function renderWelcomeMessage()
	...
	dxDrawRectangle(0, 0, screenX, screenY, tocolor(0, 0, 0,160), false)
	if guiGetVisible(bLogin) == true then
		dxDrawText( "ShoDown Roleplay", screenX/2 - 300, screenY/10, 300, 200, tocolor ( 255,20, 10, 255 ), 1.4, "pricedown" )
		dxDrawText( "Login below if you got an account\nYou can create account by clicking on register", screenX/6, screenY/4.9, screenX, screenY, tocolor ( 204, 255, 51, 255 ), 1, "default-bold" ) 
	else
		dxDrawText( "Characters", screenX/2 - 300, screenY/10, 300, 200, tocolor ( 255,20, 10, 255 ), 1.4, "pricedown" )
		dxDrawText( "Choose a character you want to play.", screenX/6, screenY/4.9, screenX, screenY, tocolor ( 204, 255, 51, 255 ), 1, "default-bold" ) 
		end
	motd = getElementData(getLocalPlayer(),"motd")
	if not motd then
		motd = "Not Set.Admins can set message using /makemotd command."
	end
	...
	if (sizey>sizey+300) then
	 ...
	 dxDrawText("Welcome Message: \n"..motd, screenX/2.5, screenY/4.9, screenX, screenY, tocolor ( 204, 255, 51, 255 ), 0.6, "bankgothic","center","top",true,true ) 
	
	 dxDrawImage(0,0, screenX, screenY , "shodown2.png", sizex,sizey,0, tocolor(255, 255, 255,alphaStep), false)
...
function createrefresh()
	...
	refreshl = guiCreateLabel(width /1.9, height /1.2,230,60, "If loginpanel fails to load or isn't working \nuse this refresh/load button.", false, wLogin)
	bRefresh = guiCreateButton(width/1.2, height /1.2, 75, 17, "Refresh", false, wLogin)
	addEventHandler("onClientGUIClick", bRefresh,refresh, false)
	end
end
...
function createMainUI(res, isChangeAccount)
	...
	if (res==getThisResource()) then
		...
		if (tutFile) or (regFile) then
			...
			error =   guiCreateLabel(width/6, height /1.4,400,260, "Errors: None", false, wLogin)
			guiLabelSetColor(error,255,0,0)
			guiSetFont(error,"birth")
			breg =  guiCreateButton(width /4, height /1.8, 75, 17, "Register", false, wLogin)
			blog =  guiCreateButton(width /6, height /1.8, 75, 17, "Back", false, wLogin)
			...
			userl = guiCreateLabel(width /6, height /2.8, 100, 50, "Username:", false, wLogin)
			usere =  guiCreateEdit(width /4, height /2.8, 100, 17, "", false, wLogin)
			addEventHandler("onClientGUIChanged",usere,checkuser,true)
			regusere = guiCreateEdit(width /4, height /2.8,194,25,"",false,loginwindow)
			addEventHandler("onClientGUIChanged",regusere,checkuser)
			guiSetVisible ( regusere, false )
			passl =   guiCreateLabel(width /6, height /2.3, 100, 50, "Password:", false, wLogin)
		
			passe =  guiCreateEdit(width /4, height /2.3, 100, 17, "", false, wLogin)
			regpasse =   guiCreateEdit(width /4, height /2.3,194,25,"",false,loginwindow)
			...
			pass2 = guiCreateLabel(width /6, height /2,79,28,"Confirm Pass:",false,loginwindow)
			guiSetVisible ( pass2, false )
			passe2 = guiCreateEdit(width /3.5, height /2,194,25,"",false,loginwindow)
			...
			chkRemember = guiCreateCheckBox(width /6, height /2,124,20,"Remember Me",false,false,loginwindow)
			chkAutoLogin =  guiCreateCheckBox(width /3.5, height /2,124,20,"Automatic Login",false,false,loginwindow)
			...
			bLogin = guiCreateButton(width /6, height /1.8, 75, 17, "Login", false, wLogin)
			addEventHandler("onClientGUIClick", bLogin, validateDetails, false)
		
			bRegister =  guiCreateButton(width /4, height /1.8, 75, 17, "Create", false, wLogin)
...
function checkpass()
	if guiGetText(regpasse) == guiGetText(passe2) then
		guiSetText(error,"Passwords entered are matching")
		guiLabelSetColor(error,0,255,0)
	else
		guiSetText(error,"Passwords entered doesn't match")
		guiLabelSetColor(error,255,0,0)
	end
end
function checkuser()
	if guiGetVisible(regusere) == true then
		...
		guiSetText(error,"Username '"..username.."' is available")
		triggerServerEvent("checkuser",getLocalPlayer(),username,1)
		guiLabelSetColor(error,0,255,0)
	else
		local username= guiGetText(usere)
		guiSetText(error,"Username '"..username.."' doesn't exits")
...
function userfound(register)
	if register ==1 then
		local username= guiGetText(regusere)
		guiSetText(error,"Username '"..username.."' is not available")	
		guiLabelSetColor(error,255,0,0)
	else
		local username= guiGetText(usere)
		guiSetText(error,"Username '"..username.."' found")	
		guiLabelSetColor(error,0,255,0)
	end
end
...
function validateDetails()
	if (source==bRegister) then
		...
		if (string.len(username)<3) then
			guiSetText(error,"Username is too short.Enter more than 3 characters")
		...
		elseif (string.find(password, ";", 0)) or (string.find(password, "'", 0)) or (string.find(password, "@", 0)) or (string.find(password, ",", 0)) then
			...
			guiSetText(error,"Your password cannot contain .")
			
		elseif (password1~=password2) then
		--	outputChatBox("The passwords you entered do not match.", 255, 0, 0)
			guiSetText(error,"The passwords you entered do not match")
		else
			showChat(true)
...
skinimage = guiCreateLabel(27,189,127,104,"Select Gender \n& Skin colour",false,creationwindow)
...
blackMales = {000,007, 014, 015, 016, 017, 018, 020, 021, 022, 024, 025, 028, 035, 036, 050, 051, 066, 067, 078, 079, 080, 083, 084, 102, 103, 104, 105, 106, 107, 134, 136, 142, 143, 144, 156, 163, 166, 168, 176, 180, 182, 183, 185, 220, 221, 222, 249, 253, 260, 262 }
	...
	if (button=="left") and (state=="up") then
		...
		elseif (source==prevSkin) then
			...
			if (curskin==1) then
				...
				setElementModel(getLocalPlayer(), tonumber(skin))
			if not skin then
				guiSetText(skinimage,"Select Skin Colour \n and Gender first")
