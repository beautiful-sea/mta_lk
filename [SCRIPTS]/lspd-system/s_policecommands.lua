...
function ticketPlayer(thePlayer, commandName, targetPlayerNick, amount, ...)
	...
	if (logged==1) then
		...
		if tonumber(amount) > 350 or tonumber(amount) <=0 then
			outputChatBox("Enter amount between 0-350",thePlayer)
			return
		end
		if (factionType==2) then
			if not (targetPlayerNick) or not (amount) or not (...) then
				...
				if targetPlayer then
					...
					if (distance<=10) then
						...
						triggerClientEvent( getRootElement(), "aoutput",getRootElement(), "LSPD",""..getPlayerName(thePlayer).." ticketed " ..targetPlayerName .. " " .. amount .. "$ for " .. reason.."." )
						outputChatBox("You ticketed " .. targetPlayerName .. " for " .. amount .. "$. Reason: " .. reason .. ".", thePlayer)
...
function takeLicense(thePlayer, commandName, targetPartialNick, licenseType, hours)
	outputChatBox("TES",thePlayer,255,255,255)
