...
--[[local Gate = {
	[3] = createObject(4100,1146.4619140625, -1292.5048828125, 13.641716003418,0,0,0),
	[4] = createObject(4100,1146.4619140625, -1292.5048828125, 15.641716003418,0,0,0),
}
local GateName = "LSES Impound Gate"
exports.pool:allocateElement(Gate[1])
exports.pool:allocateElement(Gate[2])
local open = false
...
local function closeDoor(thePlayer)
	moveObject(Gate[3], 1000,1146.4619140625, -1292.5048828125, 13.641716003418, 0, 0, 0)
	moveObject(Gate[4], 1000, 1146.4619140625, -1292.5048828125, 15.641716003418, 0, 0, 0)
	setTimer(ResetOpenState, 1000, 1)
end
...
local function useDoor(thePlayer, commandName, ...)
	local x, y, z = getElementPosition(thePlayer)
	local distance = getDistanceBetweenPoints3D(1146.4619140625, -1292.5048828125, 13.641716003418, x, y, z)
		
	if (distance<=10) and (open==false) then
		if (exports.global:hasItem(thePlayer, 64) or exports.global:hasItem(thePlayer, 82)) then
			outputChatBox("The " .. GateName .. " is now open!", thePlayer, 0, 255, 0)
			moveObject(Gate[3], 1000, 1570.4779052734,-1617.7227783203,14.1, 0, 0, 0)
			moveObject(Gate[4], 1000, 1588.2482910156,-1608.6553955078,14.1, 0, 0, 0)
			setTimer(closeDoor, 5000, 1, thePlayer)
		end
	end
end
